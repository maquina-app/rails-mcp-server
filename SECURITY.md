...
- **No process execution or `require`**: shell/system calls (`system`, `exec`, backticks, `spawn`), process/pseudo-terminal and native bridges (`Open3`, `IO.popen`, `Process.spawn`, `PTY`, `Fiddle`, `FFI`, sockets), and dynamic dispatch aimed by name at an execution/eval sink (`send`/`const_get` → `system`/`spawn`/`Open3`/…) are rejected. `require` and `require_relative` are refused entirely — `bin/rails runner` has already loaded Rails and the stdlib inspection code needs, and every dangerous stdlib escape has to be required first, so blocking all requires removes that class of bypass at the source
